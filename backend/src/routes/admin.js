...
router.post('/messages/:teacherId', (req, res) => {
  ...
  const id = uuid();
  db.prepare(`INSERT INTO messages (id, teacher_id, sender, text, read_by_teacher, read_by_admin) VALUES (?, ?, 'admin', ?, 0, 1)`)
    .run(id, req.params.teacherId, text.trim());
...
router.post('/broadcast', (req, res) => {
  ...
  if (io) {
    ...
    io.to('admin').emit('new_message', sent[0] || null);
  }
