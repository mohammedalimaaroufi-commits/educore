...
router.post('/session', (req, res) => {
  ...
  const session = db.prepare(`SELECT ats.* FROM attendance_sessions ats JOIN classes c ON ats.class_id = c.id
                               WHERE ats.id = ? AND c.teacher_id = ?`).get(session_id, req.teacherId);
  if (!session) return res.status(404).json({ error: 'الجلسة غير موجودة' });

  const upsert = db.prepare(`INSERT INTO attendance_records (id, session_id, student_id, status) VALUES (?, ?, ?, ?)
                              ON CONFLICT(session_id, student_id) DO UPDATE SET status = excluded.status`);
  const saveAll = db.transaction((items) => { for (const it of items) upsert.run(uuid(), session_id, it.student_id, it.status); });
  saveAll(records || []);
